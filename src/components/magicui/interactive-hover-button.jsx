...
export const InteractiveHoverButton = React.forwardRef(({ children, className, ...props }, ref) => {
  return (
    (<button
      ref={ref}
      className={cn(
        "group relative w-auto cursor-pointer overflow-hidden rounded-full border border-red-800 bg-background p-2 px-6 text-center font-semibold",
        className
      )}
      {...props}>
      <div className="flex items-center gap-2 justify-center text-red-800">
        <div
          className="h-2 w-2 rounded-full bg-red-800 transition-all duration-300 group-hover:scale-[100.8] "></div>
        <span
